...
//
// This file is generated by `diesel print-schema` and must not be hand-edited. It uses
// 4-space indentation, which conflicts with the repository's `tab_spaces = 2` rustfmt
// setting; rather than reformat it (which would make every regeneration a large spurious
// diff), the `cargo fmt --check` gate skips this path.
// Regenerate with: diesel print-schema > src/schema.rs   (run from src-tauri/)
