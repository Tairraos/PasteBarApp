...
- A stale `vite.config.mts.timestamp-*.mjs` build artifact on disk (gitignored, but present) that referenced nearly every package and produced false "in use" results until it was deleted.
  ...
  Packages referenced **only by vendored code that nothing reaches** are deliberately kept, because deleting them would only move the problem into `tsc` (the files are still on disk and still type-checked): `react-youtube`, `emery`, `@emotion/css`, `@react-aria/utils`, `@react-stately/utils`, `react-twitter-embed`, and the two Tauri plugin API wrappers. They are blocked on a decision about the unreachable vendored files that import them (GOLDEN-RULES R7), not on a decision about dependencies.

  A correction worth recording, because the first version of this entry got it wrong: `@radix-ui/react-{context-menu,label,popover,select,separator}` were initially listed in that group and are **not** in it. They are imported by `src/components/ui/{context-menu,label,popover,select,separator}.tsx`, which `src/components/ui`'s barrel re-exports with `export * from './…'` — so the files are reachable and the packages ship in the bundle. The scanner that produced the claim only followed `import`, not `export … from`, and a barrel re-export is exactly the shape that hides a real dependency. `scripts/harness/vendored-imports.mjs` now handles both forms, and reports **zero** removable dependencies in either manifest.
  `行为变更 | 无`
  `状态 | ✅ 已修复 (W2, 依赖清理)`
  `修复说明 |` 102 declarations removed in total across two passes: 95 in the first (root 120 → 65 deps, UI 129 → 89) and 7 more when the end-state was re-verified (`facepaint`, `events`, `@types/events`, and the stale `@types/linkify-it`, which v6 replaced with its own types but which still described v3's API — including a callable default export that no longer exists). `npm ci` drops from ~1476 to ~1158 packages: **over 300 fewer in `node_modules`**, which is the supply-chain surface that actually mattered. It also deleted 22 of the 51 production advisories outright (ISSUE-032) — they were not fixed, they were shipped by packages nothing imported. `vite build` exits 0, 49 frontend tests and 31 Rust tests pass, 13 gates green, and `vendored-imports.mjs --unused` reports **zero** removable declarations in either manifest.
...
- `redux` — imported by vendored `react-arborist`. Reaches the root only through `react-dnd-html5-backend → dnd-core → redux`.
  None of these is declared, so nothing records that the app depends on them. They work until an upstream package drops or reorganises its own dependency, at which point the build fails with a module-resolution error pointing at app code that did nothing wrong. The audit prune very nearly demonstrated this: removing 300+ packages was one unlucky deletion away from taking `redux` with it.
...
### ISSUE-037 · Backup archive contained the entire clipboard history, and copied the database unsafely

`ID | ISSUE-037`
`位置 | src-tauri/src/commands/backup_restore_commands.rs:94 (create_backup), :153 (image dirs), :546 (get_data_paths)`
`类型 | BUG`
`风险等级 | P2`
`影响范围 | Backup size and restore semantics; database consistency under WAL`
`现象与依据 |` Three distinct defects sat in one function:

- **The archive held the whole clipboard history.** History is bulk transient data — it is what makes a backup large, it is regenerated by using the app, and restoring it is not what a user means by "restore my data". Every backup carried all of it.
- **The database was copied with `fs::read` on the live `.data` file.** Under WAL a writer can be mid-commit, so the copy can capture a torn write and produce a corrupt backup. This was latent — it depends on whether a write is in flight during the read — which is exactly what makes it dangerous: it fails rarely, and the failure appears at restore time, long after the backup that caused it.
- **`get_data_paths` reported a "history-images" directory that does not exist.** The real directory is `clipboard-images`. Nothing consumed the field yet, so no user saw it, but it is the field a UI would display, and a confidently wrong path is worse than an absent one.
...
### ISSUE-038 · `src-tauri/target` grew to 6.4 GB, and release builds were assembled by hand

`ID | ISSUE-038`
`位置 | scripts/build-app.mjs (new), package.json:11-15`
`类型 | DEBT`
`风险等级 | P3`
`影响范围 | Build reproducibility and disk usage`
`现象与依据 |` `src-tauri/target` reached **6.4 GB** of incremental state that nothing removed. Separately, producing a release meant typing `tauri build` with a release config _and_ remembering to bump the version in two manifests _and_ knowing that Tauri reads one of them for the bundle name, the DMG filename and `Info.plist`. Each step could be skipped independently, and skipping the version bump produces a binary whose version does not match its filename — the failure that is hardest to notice and most likely to be shipped.
`建议方案 |` Add `scripts/build-app.mjs` as the release path: bump the patch version in both manifests, build, collect `target/PasteBar.app` and `target/PasteBar_<version>_<arch>.dmg`, then remove intermediates. It keeps Cargo's `release/` cache so the next build stays fast (measured 6.4 GB → 1.9 GB). Two traps had to be handled explicitly: the previous bundle output is deleted _before_ each build, because otherwise a build that fails at bundling leaves the old `.app` in place and the wrapper would deliver a stale binary labelled with the new version; and when Tauri's `bundle_dmg.sh` fails (it calls `bless` and AppleScript, needing privileges a sandbox may not grant, with no way to pass `--sandbox-safe` in Tauri 1.x) the wrapper builds the DMG itself via a plain `hdiutil` sequence.
`行为变更 | 无（构建流程） |
`状态 | ✅ 已修复 |
`所属阶段 | 6 (follow-up)`
...
### ISSUE-039 · Deleted rows never returned their disk space; no VACUUM existed

`ID | ISSUE-039`
`位置 | src-tauri/src/services/maintenance_service.rs (new), src-tauri/src/cron_jobs.rs:14`
`类型 | DEBT`
`风险等级 | P3`
`影响范围 | Database file growth on long-lived installs`
`现象与依据 |` `grep -rn "VACUUM" src-tauri/src/` returned nothing. Deleting rows frees pages inside the SQLite file but not to the filesystem, and this app deletes constantly — every history clear, every retention sweep. A database that had been cleared many times kept every page it had ever allocated, and its file only grew.
`建议方案 |` Run `VACUUM` from the existing `clokwerk` scheduler, with the policy in a service so it is testable: at most once per **120 hours**, tracked in a new `maintenance_log` table (`task`, `last_run_at`, `run_count`) so the cooldown survives restarts. A **failed** attempt leaves the timestamp untouched and the next hourly tick retries; only a success starts the cooldown. The asymmetry is the point — if failure armed the cooldown, a transient lock would silently postpone maintenance by five days, and a transient lock is exactly when retrying soon is correct. `VACUUM` takes an exclusive lock, so it blocks by deliberate choice; both callers are already off the UI thread.
`行为变更 | 有（新增后台维护任务；数据库文件会在维护时被重建） |
`状态 | ✅ 已修复 |
`所属阶段 | 6 (follow-up)`
