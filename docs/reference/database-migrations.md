...
`db::init(app)` is called from the Tauri app `.setup` closure at `main.rs:1061`.
...
`.plugin(clipboard::init())` at `main.rs:1401`, and its plugin setup immediately spawns the
...
migration run) only happens later, at `main.rs:1061`.
